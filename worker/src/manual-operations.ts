...
function githubDispatchError(
  ...
  if (status === 401 || /bad credentials/i.test(detail)) {
    return (
      ...
      "مقدار Secret باید خود توکن تولیدشده باشد."
    );
  }

  if (status === 403 || /resource not accessible/i.test(detail)) {
    return (
      "توکن به مخزن دسترسی کافی ندارد. " +
      "مجوز Actions: Read and write را بررسی کنید."
    );
  }

  if (status === 404) {
    return (
      "مخزن یا Workflow پیدا نشد. " +
      "نام مخزن و فایل manual-operations.yml را بررسی کنید."
    );
  }

  if (status === 422) {
    return (
      "GitHub درخواست Workflow را نپذیرفت. " +
      "شاخه main و ورودی‌های Workflow را بررسی کنید."
    );
  }
...
export async function dispatchManualOperation(
  ...
  if (active) {
    throw new Error(
      `عملیات دیگری هنوز در وضعیت «${active.status}» است. ` +
        `شناسه: ${active.operation_id.slice(0, 8)}. ` +
        "ابتدا وضعیت همان عملیات را بررسی کنید.",
    );
  }
  ...
  if (!token) {
    throw new Error(
      "Secret GITHUB_ACTIONS_TOKEN تنظیم نشده است.",
    );
  }
